...
import { selectSupportedDepartures, selectSupportedJourneys } from "../src/domain.ts";
import {
...
import { createResRobotDeepLink } from "../src/resrobot-deep-link.ts";
import { departureSchema, journeyOptionSchema, stopCandidateSchema } from "../src/schemas.ts";

test("filters stop candidates geographically", async () => {
    ...
    stopGroups: [
      {
        ...
        transport_modes: ["BUS"],
        stops: [{ id: "101", name: "Kronan", lat: 65.5775, lon: 22.1905 }],
      },
      {
        id: "740000999",
        name: "Stockholm",
  ...
  assert.deepEqual(
    trafiklabStops.map((stop) => stop.stopId),
    ["740000101"],
  );
  stopCandidateSchema.array().parse(trafiklabStops);
});

test("filters nearby stops by bus product", async () => {
...
test("normalizes operators and excludes unsupported or mixed journeys when filtered", async () => {
  ...
  assert.deepEqual(
    selectedJourneys.map((journey) => journey.id),
    ["llt", "norrbotten", "mixed"],
  );
  ...
  assert.deepEqual(
    lltJourneys.map((journey) => journey.id),
    ["llt"],
  );
});

test("filters departures by supported and selected operator", async () => {
  ...
  assert.deepEqual(
    selectedDepartures.map((departure) => departure.operator),
    ["Luleå Lokaltrafik", "Länstrafiken Norrbotten"],
  );
  departureSchema.array().parse(selectedDepartures);
  assert.deepEqual(
    selectSupportedDepartures(departures, {
      maxResults: 10,
      operator: "Luleå Lokaltrafik",
    }).map((departure) => departure.operator),
    ["Luleå Lokaltrafik"],
  );
});

test("uses the ResRobot longitude parameter", async () => {
...
test("bounds ResRobot overfetch without upstream operator filtering", async () => {
  ...
  assert.deepEqual(
    journeyRequestCounts.map(({ numF, numB }) => ({ numF, numB })),
    [
      { numF: "3", numB: "0" },
      { numF: "6", numB: "0" },
      { numF: "6", numB: "0" },
...
test("creates independent ResRobot links without credentials", async () => {
  const stopLink = new URL(
    createResRobotDeepLink({
...
test("attaches a verification link to live journeys", async () => {
  ...
  const linkedJourney = (
    await linkedJourneyProvider.journeyOptions({
      origin: { kind: "stop", stopId: "740000101" },
      destination: { kind: "stop", stopId: "740000201" },
      at: "2026-09-06T06:00:00.000Z",
      arriveBy: false,
...
function tripFixture(id: string, operators: string[]) {
  const legs = operators.map((operator, index) => ({
    ...
    duration: "PT10M",
    Origin: endpoint(
      index === 0 ? "Kronan" : "Centrum",
      `74000010${index}`,
      65.5775,
      22.1905,
      index === 0 ? "08:04:00" : "08:16:00",
    ),
    Destination: endpoint(
      index === operators.length - 1 ? "Smedjegatan" : "Centrum",
      `74000020${index}`,
      65.5845,
      22.154,
      index === operators.length - 1 ? "08:25:00" : "08:14:00",
    ),
