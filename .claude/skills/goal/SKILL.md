---
name: goal
description: Run the execution loop on a goal spec for hk-llt — plan,
...
# /goal — execution loop for hk-llt
...
- hk-llt uses the umbrella layout (`.bare` + `main/` trunk). Classic
  checkout → run `hk migrate hk-llt` first.
...
1. **Ledger**: create `loop_state.md` at the hk-llt umbrella root
   ...
   Paste the spec verbatim, then write the header block hk reads — four
   one-line fields at the top of the ledger, before any heading:
   ...
   `Issue:` is how `hk goals` joins this loop to its goal issue; omit it
   only when the goal genuinely has no issue. Keep `Phase:` and
   `Iteration:` current — they are the loop's status as everyone else
   sees it.
...
3. **Worktree**: `git -C <umbrella root> worktree add loop-<slug> -b
   ...
   install, project generation) so the gate can run. Record the branch in
   the header's `Branch:` field.
...
2. **Verifier** — fresh subagent, blind to the Writer's reasoning. It
   runs `hk verify hk-llt --worktree loop-<slug> --json` (targeted
